...
parse s (x:xs)
    | x == "$ cd /" = parse "/" xs
    | (length parole == 3) && (parole !! 2 /= "..") = s : parse (s ++ (parole !! 2) ++ "/") xs
    | (length parole == 3) && (parole !! 2 == "..") = s : parse (unify $ split s) xs
    | (parole !! 1 == "ls") || (head parole == "dir") = parse s xs
    | otherwise = (s ++ head parole) : parse s xs
